...
lorawan_status_t LoRaWANStack::state_controller(device_states_t new_state)
{
    ...
    switch (new_state) {
        case DEVICE_STATE_IDLE:
        	tr_debug("AK: *** state_controller() DEVICE_STATE_IDLE");
            process_idle_state(status);
            break;
        case DEVICE_STATE_CONNECTING:
        	tr_debug("AK: *** state_controller() DEVICE_STATE_CONNECTING");
            process_connecting_state(status);
            break;
        case DEVICE_STATE_JOINING:
        	tr_debug("AK: *** state_controller() DEVICE_STATE_JOINING");
            process_joining_state(status);
            break;
        case DEVICE_STATE_CONNECTED:
        	tr_debug("AK: *** state_controller() DEVICE_STATE_CONNECTED");
            process_connected_state();
            break;
        case DEVICE_STATE_SCHEDULING:
        	tr_debug("AK: *** state_controller() DEVICE_STATE_SCHEDULING");
            process_scheduling_state(status);
            break;
        case DEVICE_STATE_STATUS_CHECK:
        	tr_debug("AK: *** state_controller() DEVICE_STATE_STATUS_CHECK");
            process_status_check_state();
            break;
        case DEVICE_STATE_SHUTDOWN:
       	 tr_debug("AK: *** state_controller() DEVICE_STATE_SHUTDOWN");
