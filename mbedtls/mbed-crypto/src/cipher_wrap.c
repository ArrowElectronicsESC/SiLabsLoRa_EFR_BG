...
{
    return _mbedtls_aes_crypt_ecb( (mbedtls_aes_context *) ctx, operation, input, output );
}
...
{
    return _mbedtls_aes_setkey_dec( (mbedtls_aes_context *) ctx, key, key_bitlen );
}

static int aes_setkey_enc_wrap( void *ctx, const unsigned char *key,
                                unsigned int key_bitlen )
{
    return _mbedtls_aes_setkey_enc( (mbedtls_aes_context *) ctx, key, key_bitlen );
}
...
{
    _mbedtls_aes_free( (mbedtls_aes_context *) ctx );
    mbedtls_free( ctx );
}
